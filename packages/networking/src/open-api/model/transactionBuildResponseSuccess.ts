/**
 * Radix Gateway API
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 0.9.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { RequestFile } from './models';
import { TransactionBuild } from './transactionBuild';
import { TransactionBuildResponse } from './transactionBuildResponse';
import { TransactionBuildResponseSuccessAllOf } from './transactionBuildResponseSuccessAllOf';

export class TransactionBuildResponseSuccess extends TransactionBuildResponse {
    'transactionBuild': TransactionBuild;

    static discriminator: string | undefined = undefined;

    static attributeTypeMap: Array<{name: string, baseName: string, type: string}> = [
        {
            "name": "transactionBuild",
            "baseName": "transaction_build",
            "type": "TransactionBuild"
        }    ];

    static getAttributeTypeMap() {
        return super.getAttributeTypeMap().concat(TransactionBuildResponseSuccess.attributeTypeMap);
    }
}

