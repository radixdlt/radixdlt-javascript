...
const create = () => {
  ...
  const revealMnemonic = (): Observable<MnemomicT> =>
    wallet$.pipe(map((wallet: WalletT): MnemomicT => wallet.revealMnemonic()))
      ...
      .map(txHistory => ({
        ...
        transactions: txHistory.transactions.map(
          (transaction): ExecutedTransaction =>
            decorateSimpleExecutedTransactionWithType(transaction, address),
        ),
      }))
  }
    ...
    return activeAccount.pipe(
      ...
      mergeMap((account: AccountT) => {
        ...
        if (recipients.length != 1)
          return throwError(() =>
            Error('Failed to decrypt message. Multiple recipients.'),
          )
  ...
  const methods = {
    ...
    login: (password: string, loadKeystore: () => Promise<KeystoreT>) => {
      ...
      void SigningKeychain.byLoadingAndDecryptingKeystore({
        ...
        signingKeychainResult.map((signingKeychain: SigningKeychainT) => {
          walletSubscription = networkSubject.subscribe(network => {
            const wallet = Wallet.create({
              signingKeychain,
              network,
    ...
    transactionStatus: (
      ...
      trigger.pipe(
        ...
        mergeMap(([api, network]) => api.transactionStatus(txID, network)),
    ...
    tokenBalances: pipe(
      ...
      andThen(result => result.map(response => response.account_balances)),
    ),
