...
export const submitTx =
	(
		...
		finalizedTx: FinalizedTransaction
			...
			radixAPI.submitSignedTransaction(network, finalizedTx).map(
				(pendingTx: PendingTransaction) => {
					log.debug(`Submitted transaction with txID=${pendingTx.txID.toPrimitive()}`)
					track({
					...
					log.error(
						`Submission of signed transaction to API failed with errors: ${e.map(e => e.message)}`,
					)
					trackError({
						errors: e,
