...
import { AccountAddress, ValidatorAddress } from '@account'
...
export const makeBaseValidatorSubstateFromBuffer =
  <SST extends SubStateType>(substateType: SST) =>
  (
    bufferReader: BufferReaderT,
    ...
    combine([
      ...
      .map((partial): BaseValidatorSubstate<SST> => {
...
export const makeBaseStakeSubstateFromBuffer =
  <SST extends SubStateType>(substateType: SST) =>
  (
    bufferReader: BufferReaderT,
    lengthData: Buffer,
    ...
    makeBaseValidatorSubstateFromBuffer(substateType)(bufferReader).andThen(
      base =>
        ...
        ]).map((resList): BaseStakingSubstate<SST> => {
