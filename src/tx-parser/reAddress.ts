...
const fromBufferReader = (
  ...
  bufferReader
    ...
    .andThen((aa): Result<REAddressT, Error> => {
      ...
      switch (reAddressType) {
        case REAddressType.SYSTEM:
          const systemAddress: REAddressSystem = {
            ...
            toString: () => `REAddressType.SYSTEM (Always empty)`,
          }
        ...
        case REAddressType.RADIX_NATIVE_TOKEN:
          const nativeToken: REAddressNativeToken = {
            reAddressType,
            toBuffer: () => reAddressTypeBuf,
            toString: () => `REAddressType.RADIX_NATIVE_TOKEN (Always empty)`,
          }
        ...
        case REAddressType.HASHED_KEY_NONCE:
          return bufferReader.readNextBuffer(26).map(
            (lower26Bytes): REAddressHashedKeyNonce => ({
              reAddressType,
              lower26Bytes,
              toBuffer: () => Buffer.concat([reAddressTypeBuf, lower26Bytes]),
              toString: () =>
        ...
        case REAddressType.PUBLIC_KEY:
          return bufferReader
            .readNextBuffer(33)
            .andThen(pubKeyBytes => PublicKey.fromBuffer(pubKeyBytes))
            .map(
              (publicKey: PublicKeyT): REAddressPublicKey => ({
                reAddressType,
                publicKey,
                toBuffer: () =>
                  Buffer.concat([
