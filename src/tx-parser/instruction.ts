...
import { BufferReaderT, Byte } from '@util'
...
const parseFromBufferReader = (
  ...
  return bufferReader
    ...
    .andThen((ii): Result<InstructionT, Error> => {
        ...
        case InstructionType.UP:
          return parseSubstate().map(
            (substate): Ins_UP => ({
              instructionType,
              substate,
              toBuffer: () => Buffer.concat([insBuf, substate.toBuffer()]),
              toString: () => `UP(${substate.toString().trimStart()})`,
              toHumanReadableString: () => {
                if (!substate.toHumanReadableString) {
                  return `UP(${substate.toString().trimStart()})`
                }
                return `UP(${substate.toHumanReadableString().trimStart()})`
              },
            }),
          )
        case InstructionType.DOWN:
        case InstructionType.READ:
          return SubstateId.parseFromBufferReader(bufferReader).map(
            (substateId: SubstateIdT) => ({
              instructionType,
              substateId,
              toBuffer: () => Buffer.concat([insBuf, substateId.toBuffer()]),
              toString: () =>
                `${InstructionType[instructionType]}(${substateId.toString()})`,
            }),
          )
        case InstructionType.LDOWN:
          return bufferReader
            .readNextBuffer(2)
            .map((substateIndexBytes): Ins_LDOWN => {
              const substateIndex = substateIndexBytes.readUInt16BE(0)
              return {
                substateIndex,
                instructionType,
                toBuffer: () => Buffer.concat([insBuf, substateIndexBytes]),
                toString: () => `LDOWN(${substateIndex.toString()})`,
              }
            })
        case InstructionType.MSG:
          return Bytes.fromBufferReader(bufferReader).map(
            (bytes: BytesT): Ins_MSG => ({
              instructionType,
              bytes,
              toBuffer: () => Buffer.concat([insBuf, bytes.toBuffer()]),
        ...
        case InstructionType.SIG:
          return TxSignature.fromBufferReader(bufferReader).map(
            (signature: TXSig): Ins_SIG => ({
              instructionType,
              signature,
              toBuffer: () => Buffer.concat([insBuf, signature.toBuffer()]),
              toString: () => `SIG(0x${signature.toBuffer().toString('hex')})`,
            }),
          )
        case InstructionType.SYSCALL:
          return Bytes.fromBufferReader(bufferReader).map(
            (callData: BytesT): Ins_SYSCALL => ({
              instructionType,
              callData,
              toBuffer: () => Buffer.concat([insBuf, callData.toBuffer()]),
              toString: () => `SYSCALL(${callData.toString()})`,
            }),
          )
        ...
        case InstructionType.HEADER:
          return combine([
            ...
            .map(resList => {
              ...
              const buffer = Buffer.concat([versionBuf, flagBuf])
            ...
            .map(
              (partial): Ins_HEADER => ({
                ...partial,
                instructionType,
                toBuffer: () => Buffer.concat([insBuf, partial.buffer]),
                toString: () =>
