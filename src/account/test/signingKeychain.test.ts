...
import { KeystoreT, Mnemonic, PrivateKey, PublicKeyT } from '@crypto'
import { combineLatest, Subscription, throwError } from 'rxjs'
import { LogLevel, restoreDefaultLogLevel } from '@util'
...
describe('signingKeychain_type', () => {
  it('can be created via keystore', done => {
    const mnemonic = Mnemonic.generateNew()
  ...
  describe('failing signingKeychain scenarios', () => {
    ...
    it('save errors are propagated', done => {
      const mnemonic = Mnemonic.generateNew()
      const password = 'super secret password'
    ...
    it('load errors are propagated', done => {
      const password = 'super secret password'
  ...
  it('should derive next and switch to it if specified', done => {
    const subs = new Subscription()
    const signingKeychain = createSigningKeychain()
  ...
  it.skip('can list all accounts that has been added', done => {
