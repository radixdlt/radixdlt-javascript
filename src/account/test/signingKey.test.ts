...
describe('signingKey_type', () => {
  it('works', () => {
    ...
    const hdPath = HDPathRadix.fromString(`m/44'/1022'/2'/1/3'`)._unsafeUnwrap()
  ...
  it('radix_hd_path_hardened', async () => {
    ...
    const sig = await firstValueFrom(signingKey.signHash(hashedMessage))
  ...
  it('radix_hd_path_0H00', () => {
    ...
    const hdMasterSeed = HDMasterSeed.fromMnemonic({ mnemonic })
    const hdPath0H00 =
      HDPathRadix.fromString(`m/44'/1022'/0'/0/0`)._unsafeUnwrap()
    ...
    const mockHardwareSigningKey = (
      ...
      return {
        keyExchange: (
          ...
          return toObservable(
            signingKeyLocalHD.__diffieHellman(publicKeyOfOtherParty),
          )
        },
