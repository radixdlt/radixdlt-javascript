...
import { isPublicKey, PublicKey, PublicKeyT } from '@crypto'
...
>(
  ...
  const formatDataToBech32Convert = input.formatDataToBech32Convert ?? (b => b)
...
const fromString = <A extends AbstractAddressT>(
  ...
  return Bech32.decode(input)
    ...
    .andThen(({ bech32Data, publicKeyBytes, hrp }) =>
      combine([networkFromHRP(hrp), PublicKey.fromBuffer(publicKeyBytes)]).map(
        resultList => {
    ...
    .map((abstractAddress: A): A => {
      // Soundness check
      if (
        abstractAddress.toPrimitive().toLowerCase() !== bechString.toLowerCase()
      ) {
