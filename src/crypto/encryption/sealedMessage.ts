...
export const __validateTag: (buffer: Buffer) => Result<Buffer, Error> =
  validateLength.bind(null, sealedMessageAuthTagLength, 'auth tag')

export const __validateNonce: (buffer: Buffer) => Result<Buffer, Error> =
  validateLength.bind(null, sealedMessageNonceLength, 'nonce')
...
const sealedMessageFromBuffer = (
  ...
  return combine([
    readNextBuffer(PublicKey.compressedByteCount).andThen(PublicKey.fromBuffer),
