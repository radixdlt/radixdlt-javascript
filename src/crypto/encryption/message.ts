...
import { isString, readBuffer } from '@util'
...
const plaintextToString = (plaintext: Buffer, startAt = 2) =>
  Buffer.from(plaintext.slice(startAt).toString('hex'), 'hex').toString('utf-8')
  ...
  __validateEncryptedMessageLength(buf).andThen(
    (buffer): Result<EncryptedMessageT | PlaintextMessageT, Error> => {
      ...
      if (
        ...
        if (sealedMessageResult.isErr()) return err(sealedMessageResult.error)
