...
} from '..//elliptic-curve/hd/bip39/mnemonic'
import { LanguageT, StrengthT } from '..//elliptic-curve/hd/bip39/_types'
import { HDMasterSeed } from '..//elliptic-curve/hd/bip39/hdMasterSeed'
  ...
  describe('failing scenarios', () => {
    ...
    it('words must be checksummed', done => {
      ...
      Mnemonic.fromEnglishWords(abandonOnly).match(
        ...
        error => {
          expect(error.message).toBe('Invalid mnemonic, it is not checksummed.')
          done()
        },
      )
    })
