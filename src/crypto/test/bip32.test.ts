...
const bip32Vectors: BIP32Vector[] = [
  ...
  {
    name: 'Test vector 3',
    info: 'These vectors test for the retention of leading zeros. See https://github.com/bitpay/bitcore-lib/issues/47 and https://github.com/iancoleman/bip39/issues/58 for more information.',
  ...
  {
    name: 'Test vector 4',
    info: 'These vectors test for the retention of leading zeros. See https://github.com/btcsuite/btcutil/issues/172 for more information.',
...
describe('BIP32', () => {
  ...
  it('can be created from string', () => {
    const derivationPath = BIP32.fromString(`m/44'/0'/0'/0/0`)._unsafeUnwrap()
  ...
  it('bip32 test vectors', () => {
    bip32Vectors.forEach(vector => {
      const masterSeed = HDMasterSeed.fromSeed(Buffer.from(vector.seed, 'hex'))
      const masterNode = masterSeed.masterNode()
      vector.derivations.forEach((vectorDerivation, index) => {
        const path = BIP32.fromString(vectorDerivation.path)._unsafeUnwrap()
  ...
  describe('wuminzhe/bip44 tests', () => {
    ...
    it('test_get_addresses_from_wallet', () => {
      ...
      const first_address_hdpath = BIP32.fromString(`M/0/0`)._unsafeUnwrap()
      const first_address_pubKey =
        first_wallet.derive(first_address_hdpath).publicKey
      const first_address = publicKeyToAddress(
        first_address_pubKey.toString(false),
      )

      expect(first_address).toBe('0xF35d683226dA7CC652299Ea5F8798f1A8E8812cF')
    })
    ...
    it('test_create_wallet_from_xprv', () => {
      ...
      const firstHDPath = BIP32.fromString(`m/44'/60'/0'/0`)._unsafeUnwrap()
