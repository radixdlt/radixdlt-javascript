...
import { buffersEquals, readBuffer } from '@util'
...
const __validateNonce: (buffer: Buffer) => Result<Buffer, Error> =
  validateLength.bind(null, nonceLength, 'nonce (IV)')

const __validateTag: (buffer: Buffer) => Result<Buffer, Error> =
  validateLength.bind(null, tagLength, 'auth tag')
