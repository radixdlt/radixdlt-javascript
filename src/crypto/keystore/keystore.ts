...
import { SecureRandom, secureRandomGenerator, log, msgFromError } from '@util'
import {
  Scrypt,
  ScryptParams,
  ScryptParamsT,
...
const decrypt = (
  ...
  return AES_GCM_SealedBox.create({
    ...
    return validatePassword(password)
      ...
      .asyncAndThen(inp => Scrypt.deriveKey(inp))
      .map((derivedKey: Buffer): AES_GCM_OPEN_Input => {
        log.info(
      ...
      .map(decrypted => {
        log.debug(`Successfully decrypted Keystore with id='${keystore.id}'`)
        return decrypted
      })
  })
}
