import { resultToAsync, secureRandomUInt256, uint256Max } from '@primitives'

import { UInt256 } from '@radixdlt/uint256'

import { err, errAsync, ok, okAsync, Result, ResultAsync } from 'neverthrow'

import { SecureRandom, secureRandomGenerator } from '@util'
...
const __privateKeyFromValidatedScalar = (scalar: UInt256): PrivateKeyT => {
  const sign = (hashedMessage: Buffer): ResultAsync<SignatureT, Error> => {
    if (hashedMessage.length !== 32) {
      return errAsync(
        new Error('Incorrect length of message to sign, expected 32 bytes.'),
      )
    }
