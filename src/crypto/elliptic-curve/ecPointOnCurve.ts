...
import { ValidationWitness } from '@util'
import { bnFromUInt256, uint256FromBN } from '@primitives'
...
const pointFromCoordinates = (
  ...
  const shortWeirestrassCurve =
    thirdPartyLibEllipticSecp256k1.curve as curve.short
...
const ecPointOnCurveFromCoordinates = (
  ...
  const multiplyByScalar = (by: UInt256): ECPointOnCurveT => {
    ...
    const factorPoint = __pointOnCurveFromEllipticShortPoint(factorShortPoint)
  ...
  return {
    ...
    toBuffer,
    toString,
    equals: (other: ECPointOnCurveT): boolean => other.x.eq(x) && other.y.eq(y),
