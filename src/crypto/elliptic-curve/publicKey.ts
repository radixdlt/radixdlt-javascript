...
import { buffersEquals, msgFromError } from '@util'
import { bnFromUInt256 } from '@primitives'
...
const publicKeyFromEllipticKey = (
  ...
  const publicKey: PublicKeyT = {
    ...
    decodeToPointOnCurve: (): ECPointOnCurveT => {
      ...
      const pointOnCurveResult =
        __pointOnCurveFromEllipticShortPoint(shortPoint)
...
const fromPrivateKeyScalar = (
  ...
  const result = publicKeyFromEllipticKey(
    thirdPartyLibEllipticSecp256k1.keyFromPrivate(input.scalar.toString(16)),
  )
