...
const create = (input: MnemonicProps): Result<MnemomicT, Error> => {
  ...
  return strengthFromWordCount(wordCount)
    .andThen((strengthFromWC: StrengthT): Result<ValidationWitness, Error> => {
      if (strengthFromWC !== input.strength) {
        const errMsg = `Mismatch between 'words' and 'strenght'.`
        log.error(errMsg)
        return err(new Error(errMsg))
      }
      if (entropyInBitsFromWordCount(wordCount) !== input.entropy.length * 8) {
        const errMsg = `Mismatch 'words' and 'entropy'.`
        log.error(errMsg)
        return err(new Error(errMsg))
      }
      if (
        byteCountFromEntropyStrength(input.strength) !== input.entropy.length
      ) {
