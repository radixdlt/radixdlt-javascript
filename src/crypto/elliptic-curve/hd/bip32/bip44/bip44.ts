...
const validateBIP44Component = (
    ...
    return err(
      new Error(
        `Wrong hardened value, expected component at level ${
          component.level
        } to${expected.isHardened ? '' : ' NOT'} be hardened, but it is${
          component.isHardened ? '' : ' NOT'
        }.`,
      ),
    )
  if (expected.name) {
  ...
  if (expected.index) {
    if (component.index !== expected.index) {
      return err(
        new Error(
          `Wrong index, component.index: ${component.index}, expected.index: ${
            expected.index
...
const fromString = (path: string): Result<BIP44T, Error> =>
  BIP32.fromString(path).andThen((bip32: BIP32T): Result<BIP44T, Error> => {
