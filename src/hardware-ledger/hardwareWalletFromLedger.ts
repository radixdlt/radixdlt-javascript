...
import { msgFromError, readBuffer, toObservableFromResult } from '@util'
import {
  GetPublicKeyInput,
  HardwareSigningKeyT,
  HardwareWalletT,
...
const withLedgerNano = (ledgerNano: LedgerNanoT): HardwareWalletT => {
  const getPublicKey = (input: GetPublicKeyInput): Observable<PublicKeyT> =>
    ledgerNano
      ...
      .pipe(
        mergeMap((buf): Observable<PublicKeyT> => {
          ...
          return toObservableFromResult(PublicKey.fromBuffer(publicKeyBytes))
        }),
      )
  ...
  const getVersion = (): Observable<SemVerT> =>
    ledgerNano
      ...
      .pipe(mergeMap(buf => toObservableFromResult(SemVer.fromBuffer(buf))))
  ...
  const parseSignatureFromLedger = (
    ...
    const signatureDERBytesResult =
      bufferReader.readNextBuffer(signatureDERlength)
  ...
  const doKeyExchange = (
    ...
    ledgerNano
      ...
      .pipe(
        mergeMap((buf: Buffer): Observable<ECPointOnCurveT> => {
          ...
          const sharedKeyPointLength =
            sharedKeyPointLengthResult.value.readUIntBE(0, 1)
...
const create = (
  ...
  return ledgerNano$.pipe(map((ledger: LedgerNanoT) => withLedgerNano(ledger)))
}
