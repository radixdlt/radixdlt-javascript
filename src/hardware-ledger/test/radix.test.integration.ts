  ...
  it('wallet_derive_hw_account', done => {
    ...
    subs.add(
      radix.deriveHWAccount(input).subscribe({
        next: account => {
          expect(account.hdPath!.toString()).toBe(`m/44'/1022'/0'/0/0'`)

          const publicKeyCompressedHex = account.publicKey.toString(true)
          if (
          ...
          expect((account.type as SigningKeyTypeHDT).hdSigningKeyType).toBe(
